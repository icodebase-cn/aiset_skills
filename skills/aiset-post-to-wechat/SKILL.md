...
test -f .aiset_skills/aiset-post-to-wechat/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.aiset_skills/aiset-post-to-wechat/EXTEND.md" && echo "user"
```

┌────────────────────────────────────────────────────────┬───────────────────┐
│                          Path                          │     Location      │
├────────────────────────────────────────────────────────┼───────────────────┤
│ .aiset_skills/aiset-post-to-wechat/EXTEND.md           │ Project directory │
├────────────────────────────────────────────────────────┼───────────────────┤
│ $HOME/.aiset_skills/aiset-post-to-wechat/EXTEND.md     │ User home         │
└────────────────────────────────────────────────────────┴───────────────────┘
...
| API credentials | Follow guided setup in Step 5, or manually set in `.aiset_skills/.env` |
...
```bash
# Check if aiset-markdown-to-html exists
test -f skills/aiset-markdown-to-html/SKILL.md && echo "found"
```
...
https://github.com/JimLiu/aiset_skills/blob/main/skills/aiset-markdown-to-html/SKILL.md
...
```bash
# Check project-level
test -f .aiset_skills/.env && grep -q "WECHAT_APP_ID" .aiset_skills/.env && echo "project"

# Check user-level
test -f "$HOME/.aiset_skills/.env" && grep -q "WECHAT_APP_ID" "$HOME/.aiset_skills/.env" && echo "user"
```
...
Where to save?
A) Project-level: .aiset_skills/.env (this project only)
B) User-level: ~/.aiset_skills/.env (all projects)
```
...
- Guided setup in Step 5, or manually set in `.aiset_skills/.env`
...
- A markdown-to-html skill (e.g., `aiset-markdown-to-html`)
- If not installed, the workflow will suggest installation

**Config File Locations** (priority order):
1. Environment variables
2. `<cwd>/.aiset_skills/.env`
3. `~/.aiset_skills/.env`
...
| Issue | Solution |
|-------|----------|
| No markdown-to-html skill | Install `aiset-markdown-to-html` from suggested URL |
