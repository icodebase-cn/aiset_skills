...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.aiset_skills', '.env');
  const userEnv = path.join(os.homedir(), '.aiset_skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== aiset-post-to-wechat: Permission & Environment Check ===\n');
