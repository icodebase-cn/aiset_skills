...
function loadConfig(): WechatConfig {
  const cwdEnvPath = path.join(process.cwd(), ".aiset_skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".aiset_skills", ".env");
  ...
  if (!appId || !appSecret) {
    throw new Error(
      ...
      "Set via environment variables or in .aiset_skills/.env file."
    );
  }
