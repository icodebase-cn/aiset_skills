...
test -f .aiset_skills/aiset-image-gen/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.aiset_skills/aiset-image-gen/EXTEND.md" && echo "user"
```

┌──────────────────────────────────────────────────┬───────────────────┐
│                       Path                       │     Location      │
├──────────────────────────────────────────────────┼───────────────────┤
│ .aiset_skills/aiset-image-gen/EXTEND.md          │ Project directory │
├──────────────────────────────────────────────────┼───────────────────┤
│ $HOME/.aiset_skills/aiset-image-gen/EXTEND.md    │ User home         │
└──────────────────────────────────────────────────┴───────────────────┘
...
# aiset third-party gateway
npx -y bun ${SKILL_DIR}/scripts/main.ts --prompt "一只可爱的猫" --image out.png --provider aiset
```
...
| `--provider google\|openai\|dashscope\|aiset` | Force provider (default: google) |
| `--model <id>`, `-m` | Model ID (`--ref` with OpenAI requires GPT Image model, e.g. `gpt-image-1.5`) |
| `--ar <ratio>` | Aspect ratio (e.g., `16:9`, `1:1`, `4:3`) |
| `--size <WxH>` | Size (e.g., `1024x1024`) |
| `--quality normal\|2k` | Quality preset (default: 2k) |
| `--imageSize 1K\|2K\|4K` | Image size for Google (default: from quality) |
| `--ref <files...>` | Reference images. Supported by Google multimodal, OpenAI edits (GPT Image models), and aiset (`image_url`). If provider omitted: Google first, then OpenAI, then aiset |
...
| `aiset_IMAGE_MODEL` | aiset model override (default: gemini-3-pro-image-preview) |
| `OPENAI_BASE_URL` | Custom OpenAI endpoint |
| `GOOGLE_BASE_URL` | Custom Google endpoint |
| `DASHSCOPE_BASE_URL` | Custom DashScope endpoint |
| `aiset_BASE_URL` | Custom aiset endpoint (default: `https://api.aiset.ai/v1`) |

**Load Priority**: CLI args > EXTEND.md > env vars > `<cwd>/.aiset_skills/.env` > `~/.aiset_skills/.env`

## Provider Selection

1. `--ref` provided + no `--provider` → auto-select Google first, then OpenAI, then aiset
2. `--provider` specified → use it (if `--ref`, must be `google` or `openai` or `aiset`)
