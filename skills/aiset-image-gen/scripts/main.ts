...
Options:
  ...
  --provider google|openai|dashscope|aiset  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
  --ar <ratio>              Aspect ratio (e.g., 16:9, 1:1, 4:3)
  --size <WxH>              Size (e.g., 1024x1024)
  --quality normal|2k       Quality preset (default: 2k)
  --imageSize 1K|2K|4K      Image size for Google (default: from quality)
  --ref <files...>          Reference images (Google multimodal, OpenAI edits, or aiset image_url)
...
Environment variables:
  ...
  aiset_API_KEY            aiset API key
  OPENAI_IMAGE_MODEL        Default OpenAI model (gpt-image-1.5)
  GOOGLE_IMAGE_MODEL        Default Google model (gemini-3-pro-image-preview)
  DASHSCOPE_IMAGE_MODEL     Default DashScope model (z-image-turbo)
  aiset_IMAGE_MODEL        Default aiset model (gemini-3-pro-image-preview)
  OPENAI_BASE_URL           Custom OpenAI endpoint
  GOOGLE_BASE_URL           Custom Google endpoint
  DASHSCOPE_BASE_URL        Custom DashScope endpoint
  aiset_BASE_URL           Custom aiset endpoint (default: https://api.aiset.ai/v1)

Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.aiset_skills/.env > ~/.aiset_skills/.env`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".aiset_skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".aiset_skills", ".env"));
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, aiset: null };
        currentKey = "default_model";
      } else if (
        currentKey === "default_model" &&
        (key === "google" || key === "openai" || key === "dashscope" || key === "aiset")
      ) {
        const cleaned = value.replace(/['"]/g, "");
...
async function loadExtendConfig(): Promise<Partial<ExtendConfig>> {
  ...
  const paths = [
    path.join(cwd, ".aiset_skills", "aiset-image-gen", "EXTEND.md"),
    path.join(home, ".aiset_skills", "aiset-image-gen", "EXTEND.md"),
  ];
...
function detectProvider(args: CliArgs): Provider {
  if (
    ...
    throw new Error(
      "Reference images require a ref-capable provider. Use --provider google (Gemini multimodal), --provider openai (GPT Image edits), or --provider aiset."
    );
  }
  ...
  const hasaiset = !!process.env.aiset_API_KEY;

  if (args.referenceImages.length > 0) {
    if (hasGoogle) return "google";
    if (hasOpenai) return "openai";
    if (hasaiset) return "aiset";
    throw new Error(
      "Reference images require Google, OpenAI, or aiset. Set GOOGLE_API_KEY/GEMINI_API_KEY, OPENAI_API_KEY, or aiset_API_KEY, or remove --ref."
    );
  }

  const available = [hasGoogle && "google", hasOpenai && "openai", hasDashscope && "dashscope", hasaiset && "aiset"].filter(
    Boolean
  ) as Provider[];
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, or aiset_API_KEY.\n" +
      "Create ~/.aiset_skills/.env or <cwd>/.aiset_skills/.env with your keys."
  );
}
...
async function validateReferenceImages(referenceImages: string[], provider: Provider): Promise<void> {
  for (const refPath of referenceImages) {
    if (provider === "aiset" && isHttpUrl(refPath)) {
      continue;
    }
...
async function main(): Promise<void> {
  ...
  if (!model && extendConfig.default_model) {
    ...
    if (provider === "aiset") model = extendConfig.default_model.aiset ?? null;
  }
