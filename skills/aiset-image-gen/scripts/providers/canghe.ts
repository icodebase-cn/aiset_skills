...
export function getDefaultModel(): string {
  return process.env.aiset_IMAGE_MODEL || "gemini-3-pro-image-preview";
}

function getApiKey(): string | null {
  return process.env.aiset_API_KEY || null;
}

function getBaseUrl(): string {
  const base = process.env.aiset_BASE_URL || "https://api.aiset.ai/v1";
...
async function buildContent(prompt: string, refs: string[]): Promise<aisetContentItem[]> {
  const content: aisetContentItem[] = [{ type: "text", text: prompt }];
...
function extractMessageContent(content: aisetResponse["choices"]): string {
...
export async function generateImage(prompt: string, model: string, args: CliArgs): Promise<Uint8Array> {
  ...
  if (!apiKey) throw new Error("aiset_API_KEY is required");
  ...
  if (!res.ok) {
    ...
    throw new Error(`aiset API error (${res.status}): ${err}`);
  }

  const result = (await res.json()) as aisetResponse;
  const rawContent = extractMessageContent(result.choices);
  const base64 = extractBase64FromText(rawContent);

  if (!base64) {
    throw new Error("No base64 image found in aiset response");
  }
