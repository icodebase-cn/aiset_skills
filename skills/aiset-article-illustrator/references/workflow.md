...
```bash
test -f .aiset_skills/aiset-article-illustrator/EXTEND.md && echo "project"
test -f "$HOME/.aiset_skills/aiset-article-illustrator/EXTEND.md" && echo "user"
```
...
|------------------------|--------|
| Yes (e.g., aiset-image-gen with Google) | Pass reference images via `--ref` |
