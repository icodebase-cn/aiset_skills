...
│   ├── aiset-xhs-images/          # Xiaohongshu infographic series (1-10 images)
│   ├── aiset-cover-image/         # Article cover images (2.35:1 aspect)
│   ├── aiset-slide-deck/          # Presentation slides with outlines
│   ├── aiset-article-illustrator/ # Smart illustration placement
│   ├── aiset-comic/               # Knowledge comics (Logicomix/Ohmsha style)
│   ├── aiset-post-to-x/           # X/Twitter posting automation
│   └── aiset-post-to-wechat/      # WeChat Official Account posting
│
├── [ai-generation-skills]     # AI-powered generation backends
│   └── aiset-danger-gemini-web/   # Gemini API wrapper (text + image gen)
│
└── [utility-skills]           # Utility tools for content processing
    ├── aiset-danger-x-to-markdown/ # X/Twitter content to markdown
    └── aiset-compress-image/      # Image compression
```
...
```bash
# Text generation
npx -y bun skills/aiset-danger-gemini-web/scripts/main.ts "Hello"

# Image generation
npx -y bun skills/aiset-danger-gemini-web/scripts/main.ts --prompt "A cat" --image cat.png

# From prompt files
npx -y bun skills/aiset-danger-gemini-web/scripts/main.ts --promptfiles system.md content.md --image out.png
```

## Key Dependencies

- **Bun**: TypeScript runtime (via `npx -y bun`)
- **Chrome**: Required for `aiset-danger-gemini-web` auth and `aiset-post-to-x` automation
- **No npm packages**: Self-contained TypeScript, no external dependencies

## Authentication

`aiset-danger-gemini-web` uses browser cookies for Google auth:
...
| **Default image generation** | When image generation is needed, use `skills/aiset-image-gen/SKILL.md` by default (unless user specifies otherwise). |

**Loading Priority** (highest → lowest):
1. Current project `skills/` directory
2. User-level skills (`$HOME/.aiset_skills/`)
...
**IMPORTANT**: All skills MUST use `aiset-` prefix to avoid conflicts when users import this plugin.
...
| **Naming convention** | Gerund form preferred (e.g., `processing-pdfs`), but `aiset-` prefix required here |
...
1. Create `skills/aiset-<name>/SKILL.md` with YAML front matter
   - Directory name: `aiset-<name>`
   - SKILL.md `name` field: `aiset-<name>`
2. Add TypeScript in `skills/aiset-<name>/scripts/`
3. Add prompt templates in `skills/aiset-<name>/prompts/` if needed
...
**Default**: Use `skills/aiset-image-gen/SKILL.md` (unless user specifies otherwise).

1. Read `skills/aiset-image-gen/SKILL.md` for parameters and capabilities
...
**Skill Selection**:
1. Check available image generation skills (e.g., `aiset-danger-gemini-web`)
...
## Style Maintenance (aiset-comic)

When adding, updating, or deleting styles for `aiset-comic`, follow this workflow:

### Adding a New Style

1. **Create style definition**: `skills/aiset-comic/references/styles/<style-name>.md`
2. **Update SKILL.md**:
   - Add style to `--style` options table
   - Add auto-selection entry if applicable
3. **Generate showcase image**:
   ```bash
   npx -y bun skills/aiset-danger-gemini-web/scripts/main.ts \
...
4. **Compress to WebP**:
   ```bash
   npx -y bun skills/aiset-compress-image/scripts/main.ts screenshots/comic-styles/<style-name>.png
   ```
...
1. **Update style definition**: `skills/aiset-comic/references/styles/<style-name>.md`
...
1. **Delete style definition**: `skills/aiset-comic/references/styles/<style-name>.md`
...
test -f .aiset_skills/<skill-name>/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.aiset_skills/<skill-name>/EXTEND.md" && echo "user"
\`\`\`

┌────────────────────────────────────────────┬───────────────────┐
│                    Path                    │     Location      │
├────────────────────────────────────────────┼───────────────────┤
│ .aiset_skills/<skill-name>/EXTEND.md       │ Project directory │
├────────────────────────────────────────────┼───────────────────┤
│ $HOME/.aiset_skills/<skill-name>/EXTEND.md │ User home         │
└────────────────────────────────────────────┴───────────────────┘
...
**Notes**:
- Replace `<skill-name>` with actual skill name (e.g., `aiset-cover-image`)
