...
| **writing-skills** | 创意写作与故事创作 | [salt-story](#salt-story) |
...
```bash
# 使用主角图片生成漫剧
/aiset-manga-drama generate --image /path/to/character.png --theme "校园日常" --scenes 3
...
```bash
# 基础生成（日式风格）
/aiset-manga-style-video "女孩在樱花树下读书"

# 指定风格
/aiset-manga-style-video "山水意境" --style chinese
/aiset-manga-style-video "可爱小动物" --style cartoon
/aiset-manga-style-video "乡村风景" --style ghibli

# 带参考图片
/aiset-manga-style-video "奶奶在包饺子" --style japanese --image ~/Desktop/character.png
```
...
# 发布带图片
/aiset-post-to-x "看看这个" --image photo.png
...
```bash
# 添加到 .aiset_skills/.env（项目级）或 ~/.aiset_skills/.env（用户级）
...
```bash
# 基础生成（自动检测服务商）
/aiset-image-gen --prompt "一只可爱的猫" --image cat.png

# 指定宽高比
/aiset-image-gen --prompt "风景图" --image landscape.png --ar 16:9

# 高质量（2k 分辨率）
/aiset-image-gen --prompt "横幅图" --image banner.png --quality 2k

# 指定服务商
/aiset-image-gen --prompt "一只猫" --image cat.png --provider openai

# DashScope（阿里通义万相）
/aiset-image-gen --prompt "一只可爱的猫" --image cat.png --provider dashscope

# 带参考图（仅 Google 多模态支持）
/aiset-image-gen --prompt "把它变成蓝色" --image out.png --ref source.png
```
...
```bash
/aiset-danger-gemini-web "你好，Gemini"
/aiset-danger-gemini-web --prompt "解释量子计算"
```

**图片生成：**

```bash
/aiset-danger-gemini-web --prompt "一只可爱的猫" --image cat.png
...
```bash
# 文生视频
/aiset-seedance-video create --prompt "小猫对着镜头打哈欠" --wait --download ~/Desktop

# 从本地图片生成视频
/aiset-seedance-video create --prompt "人物缓缓转头微笑" --image /path/to/photo.jpg --wait --download ~/Desktop

# 首帧+末帧视频生成
/aiset-seedance-video create --prompt "花朵从含苞到盛开" --image first.jpg --last-frame last.jpg --wait --download ~/Desktop

# 自定义参数
/aiset-seedance-video create --prompt "城市夜景延时摄影" --ratio 21:9 --duration 8 --resolution 1080p --wait --download ~/Desktop
```
...
**触发词**：盐选故事、知乎故事、付费故事、故事写作、写故事、盐选
...
```bash
# 基础视频分析
/aiset-volcengine-video-understanding /path/to/video.mp4 "描述这个视频的内容"

# 视频问答
/aiset-volcengine-video-understanding /path/to/video.mp4 "视频中出现了哪些人物？"

# 情感分析
/aiset-volcengine-video-understanding /path/to/video.mp4 "分析视频中人物的情感变化"

# 指定模型和帧率
/aiset-volcengine-video-understanding /path/to/video.mp4 "总结视频要点" --model doubao-seed-2-0-pro-260215 --fps 2
```
...
本仓库中不少技能是在宝玉老师和苍何的技能基础上进行二次修改和本地化落地，结合了实际工作流、工具链与提示词风格。源仓库地址：[https://github.com/JimLiu/baoyu-skills](https://github.com/JimLiu/baoyu-skills)和[https://github.com/freestylefly/canghe-skills](https://github.com/freestylefly/canghe-skills)。
...
**加载优先级**（高优先级覆盖低优先级）：
1. 命令行环境变量（如 `OPENAI_API_KEY=xxx /aiset-image-gen ...`）
2. `process.env`（系统环境变量）
3. `<cwd>/.aiset_skills/.env`（项目级）
4. `~/.aiset_skills/.env`（用户级）
...
```bash
mkdir -p .aiset_skills
# 将 .aiset_skills/.env 添加到 .gitignore 避免提交密钥
...
**扩展路径**（按优先级检查）：
1. `.aiset_skills/<skill-name>/EXTEND.md` - 项目级（团队/项目特定设置）
2. `~/.aiset_skills/<skill-name>/EXTEND.md` - 用户级（个人偏好设置）

**示例**：为 `aiset-cover-image` 自定义品牌配色：

```bash
mkdir -p .aiset_skills/aiset-cover-image
```

然后创建 `.aiset_skills/aiset-cover-image/EXTEND.md`：
...
```bash
HTTP_PROXY=http://127.0.0.1:7890 HTTPS_PROXY=http://127.0.0.1:7890 /aiset-danger-gemini-web "你好"
```
