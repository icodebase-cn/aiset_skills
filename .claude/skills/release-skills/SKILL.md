...
**Example Commits**:
```bash
git commit -m "feat(aiset-cover-image): add watercolor and minimalist styles"
git commit -m "fix(aiset-comic): improve panel layout for long dialogues"
...
```
Commits created:
  1. feat(aiset-cover-image): add watercolor and minimalist styles
  2. fix(aiset-comic): improve panel layout for long dialogues
...
Changes grouped by skill/module:
  aiset-cover-image:
    ...
    → Commit: feat(aiset-cover-image): add watercolor and minimalist styles
    → README updates: options table

  aiset-comic:
    - fix: panel layout for long dialogues
    → Commit: fix(aiset-comic): improve panel layout for long dialogues
    → No README updates
